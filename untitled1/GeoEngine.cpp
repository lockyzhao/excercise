...
GeoEngine::GeoEngine():model(nullptr)
{
    vboIds.clear();
}
...
GeoEngine::~GeoEngine()
{
    ...
    if (vboIds.size()>0) glDeleteBuffers(2, vboIds.data());
	//vboIds.clear();
}

void GeoEngine::init(const std::string& file_name)
{
    initializeOpenGLFunctions();


    initObjeModels(file_name);
...
//void GeoEngine::render(QOpenGLShaderProgram *program)
//{
...
void GeoEngine::render(QOpenGLShaderProgram *program)
{
    ...
    // Tell OpenGL programmable pipeline how to locate vertex position data
    int vertexLocation = program->attributeLocation("vert");
    program->enableAttributeArray(vertexLocation);
    glVertexAttribPointer(vertexLocation, 3, GL_FLOAT, GL_FALSE, 0, (const void *)offset);

    // Offset for texture coordinate
    offset = model->verts.size()*sizeof(Model::Point3f)+model->texs.size()*sizeof(Model::Point2f);

    // Tell OpenGL programmable pipeline how to locate vertex texture coordinate data
    int normalcoordLocation = program->attributeLocation("normal0");
...
void GeoEngine::initCubeGeometry()
{
...
//! [1]
    ...
    vboIds.resize(2);
    glGenBuffers(model->face_properties_num+1, vboIds.data());
...
void GeoEngine::initObjeModels(const std::string& file_name)
{
    if (vboIds.size()>0){
    ...
    if (model->verts.size()>0){
        glBindBuffer(GL_ARRAY_BUFFER, vboIds[0]);
        glBufferData(GL_ARRAY_BUFFER, model->verts.size() * sizeof(ObjModel::Point3f)+model->texs.size() * sizeof(ObjModel::Point2f)+model->norms.size() * sizeof(ObjModel::Point3f), 0, GL_STATIC_DRAW);
        glBufferSubData(GL_ARRAY_BUFFER, 0, model->verts.size() * sizeof(ObjModel::Point3f), model->verts.data());
        glBufferSubData(GL_ARRAY_BUFFER, model->verts.size() * sizeof(ObjModel::Point3f), model->texs.size() * sizeof(ObjModel::Point2f), model->texs.data());
        glBufferSubData(GL_ARRAY_BUFFER, model->verts.size() * sizeof(ObjModel::Point3f)+model->texs.size() * sizeof(ObjModel::Point2f), model->norms.size() * sizeof(ObjModel::Point3f), model->norms.data());
    }


    if (model->faces.size()>0){
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vboIds[1]);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, model->faces.size() * sizeof(ObjModel::Face),model->faces.data(), GL_STATIC_DRAW);
    }
